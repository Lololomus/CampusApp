...
export function describeUploadError(error) {
  ...
  // Nginx: слишком долго ждём тело запроса (часто медленный аплоад через туннель/моб. сеть).
  if (status === 408) {
    return 'Таймаут загрузки (408): файл не успел передаться до сервера. Попробуйте Wi‑Fi, меньший файл или повторите позже.';
  }
  // Шлюз/прокси (nginx, туннель): апстрим не ответил или оборвалось соединение при долгом POST
  if (status === 502) {
    return 'Шлюз вернул ошибку (502): сервер или туннель недоступны, либо сработал таймаут. Попробуйте позже.';
  }
  if (status === 503) return 'Сервис временно недоступен (503). Попробуйте через минуту.';
  if (status === 504) {
    return 'Таймаут шлюза (504): загрузка или обработка заняли слишком много времени. Уменьшите файл или проверьте прокси.';
  }
